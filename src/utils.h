...
{
public:
    BMPFile *m_bitmapCache = NULL;
    int8_t m_pageId = -1;
    int8_t m_eltId = -1;
    void store(int8_t pageId, int8_t eltId, BMPFile *bitmapCache)
    {
...
{
public:
    BmpCacheElt m_bmpElt[8];

    BMPFile *get(uint8_t pageId,uint8_t eltId)
    {
        for (int8_t i = 0; i < 8; i++)
        {
            if (m_bmpElt[i].m_pageId == pageId && m_bmpElt[i].m_eltId == eltId)
            {
                return m_bmpElt[i].m_bitmapCache;
            }
        }
        return NULL;
    };

    int8_t getNext(uint8_t pageId, uint8_t eltId)
    {
        for (int8_t i = 0; i < 8; i++)
        {
            if (m_bmpElt[i].m_bitmapCache == NULL)
            {
                return i;
            }
            else if (m_bmpElt[i].m_pageId == pageId && m_bmpElt[i].m_eltId == eltId)
            {
                return i;
            }
        }
    ...
    void store(int8_t pageId, int8_t eltId, BMPFile *bitmapCache)
    {
        int8_t id = getNext(pageId, eltId);
        if (id < 0)
        {
            DEBUGLOG("not more cache avail");
            return;
        }
        if (m_bmpElt[id].m_bitmapCache != NULL)
        {
            delete m_bmpElt[id].m_bitmapCache;
        }
        m_bmpElt[id].m_pageId = pageId;
        m_bmpElt[id].m_eltId = eltId;
        m_bmpElt[id].m_bitmapCache = bitmapCache;
        return;
    };
    ...
    void myDelete()
    {
        for (int8_t i = 0; i < 8; i++)
        {
            if (m_bmpElt[i].m_bitmapCache != NULL)
            {
                delete m_bmpElt[i].m_bitmapCache;
                m_bmpElt[i].m_bitmapCache = NULL;
            }
        }
        return;
    }
